...
exports.mailerFun = async (email, fName, userId) => {
  ...
  console.log(
    "=====",
    `${baseurl + "/user/verify/" + userId + "/" + uniqueString}`
  );
  // send mail with defined transport object
  let info = await transporter.sendMail({
    from: '"saurabh Singh" <process.env.FROM>', // sender address
    to: `${fName} <${email}>`, // list of receivers /name /email
    ...
    html: `<div style="font-family: Helvetica,Arial,sans-serif;min-width:1000px;overflow:auto;line-height:2">
        <div style="margin:50px auto;width:70%;padding:20px 0">
          ...
          <p>Thank you . Use the following Link to complete your Sign Up procedures</p>
          <h2 style="background: #4044ee;margin: 0 auto;width: max-content;padding: 0 10px;color: #fff;border-radius: 4px;">
             <a href=${baseurl + "/user/verify/" + userId + "/" + uniqueString
  ...
  console.log(data, "===========");
  data.uniqueString = newVerification
  data.save()
};